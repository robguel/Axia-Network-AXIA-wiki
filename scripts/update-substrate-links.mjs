/**
 * This script should replace axlib rustdoc links in the md files for the latest version of axlib.
 */
...
(async () => {
  ...
  const testAxlibDocsVersion = async (tag) => {
    let tagTemp = tag;
    let pass = false;
    
    while (!pass) {
      let testUrl = `http://axlib.dev/rustdocs/${tagTemp}/sc_service/index.html`;
      try {
        ...
        if (tagTemp.length === 0) {
          console.log(`NOTE: '${tag}' and variants of its prefixes are not valid Axlib rustdoc version numbers. No links to update!`);
          process.exit(0);
        }
      }
    }
  ...
  await testAxlibDocsVersion(latestTag);
  await testAxlibDocsVersion(pastTag);
  ...
  for (const file of dir) {
    ...
    for (const line of readFile.split("\n")) {
      if (line.indexOf("axlib.dev/rustdocs") !== -1) {
