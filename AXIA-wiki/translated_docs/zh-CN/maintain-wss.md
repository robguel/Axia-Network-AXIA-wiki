...
You might want to host a node on one server and then connect to it from a UI hosted on another, e.g. [AXIA-JS UI](https://AXIA.js.org/apps). This will not be possible unless you set up a secure proxy for websocket connections. Let's see how we can set up WSS on a remote Axlib node.
...
不论是个通用的 Axlib ，AXIALunar 节点，还是你自己的私有链，他们都默认使用相同的 websocket 连接: 本地主机上的 9944 端口。对于这个示例，我们会设置个 AXIALunar 同步节点(非验证人)。

在您服务商选创建务器提或在本地创建 (首选)。 假设您正在使用 Ubuntu 18.04。然后安装 Axlib 并构建节点。
...
Now you have a secure remote connect setup for your Axlib node.
