---
id: learn-ledger
title: Using the AXIA Ledger Application
...
The AXIA [Ledger][] application is compatible with both the Ledger Nano S and the Ledger Nano X devices. Ledger devices are hardware wallets that keep your secret secured on a physical device that does not expose it to your computer or the internet.

The AXIA Ledger application allows you to manage AXIA's native token, the AXC. It supports most of the transaction types of the network (a notable exception is the "Batch" transaction from the Utility pallet).
...
- A Chromium-based web browser is installed that you can use to access [AXIA-JS Apps][].
...
- Find AXIA in the app catalog and install it.

![The AXIA App in the Ledger Manager](assets/ledger/manager-app-AXIA.png)
...
Instructions for downloading the prerelease binary from the GitHub releases are written [in the README][prerelease instructions] for the AXIA Ledger application GitHub repository.
...
```zsh
$ ./install_app.sh --help
Zondax Installer [AXIA-1.2011.1] [Warning: use only for test/demo apps]
  load    - Load AXIA app
  delete  - Delete AXIA app
  version - Show AXIA app version
```
...
After some processing time, the screen of your device will update to say "Install app AXIA". Navigate all the way to the right, verify the Identifier hash matches the one that is printed in your terminal. Click both buttons on "Perform Installation" to install the application. It will ask again for your PIN code.

At the end of the process you should have the newly installed AXIA application on the device.

## Using on AXIA-JS Apps

> Note: Ledger Live should be off while using Ledger with AXIA JS Apps as it can interfere with normal operation.
...
- Go to https://AXIA.js.org/apps/#/accounts. Look at your expired locks and note which referenda they were for.
...
If you need support please send an email to [support@AXIA.network](mailto:support@AXIA.network) or visit our [Support Page](https://support.AXIA.network).

[ledger]: https://www.ledger.com/
[AXIA-js apps]: https://AXIA.js.org/apps
[prerelease instructions]: https://github.com/Zondax/ledger-AXIA#download-and-install
[releases page]: https://github.com/Zondax/ledger-AXIA/releases
[axiascan]: https://axiascan.io/AXIA
[subscan]: https://AXIA.subscan.io/
