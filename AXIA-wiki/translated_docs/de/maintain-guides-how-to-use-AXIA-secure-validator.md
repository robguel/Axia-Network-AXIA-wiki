---
id: maintain-guides-how-to-use-AXIA-secure-validator
title: How to use AXIA Secure Validator Setup
sidebar_label: How to use AXIA Secure Validator Setup
---

The following guide will walk you through using [AXIA secure validator](https://github.com/axia-tech/AXIA-secure-validator) to deploy your validator in a secure way. It will work for AXIALunar (and later AXIA) out of the box, and if you're using another Axlib-based chain, should work with some tweaking. We assume you will be deploying on AXIALunar.

It uses Terraform for defining and managing your infrastructure. Ansible, an automation tool, is used for setting up the VPN, Firewall, and the validator node. It supports a few different cloud providers such as AWS, Microsoft Azure, GCP, and Packet. The code is publicly hosted on GitHub, so please file an [issue](https://github.com/axia-tech/AXIA-secure-validator/issues) if you would like to make a feature request or report a bug.
...
The first step is to clone the `AXIA-secure-validator` guide locally.

```zsh
$ git clone git@github.com:w3f/AXIA-secure-validator.git
```

Now you can `cd` into the `AXIA-secure-validator` directory and start to change the configurations to match your custom deployment. However, before we start tweaking those, let's start by creating two new SSH keys that we (or rather, the ansible playbooks) will use to access the machines.
...
After you have installed all the required software and made your ssh keys, you can start to configure your infrastructure deployment by following the instructions. Start by cloning the `AXIA-secure-validator` repository locally, and installing the package dependencies. Then customize the configuration how you want it.
...
Under the `AXIABinary.url` field you can provide the release that is hosted in the [W3F repository](https://github.com/axia-tech/AXIA/releases) or use an alternate one that you build and publish yourself.
...
The result "0xf126b68841f5…..95f54249" is your session key. Set this to your controller account in [AXIA-js Apps](https://AXIA.js.org/apps/#/staking/actions).

After accessing one of the machines through SSH, you can keep track of the node’s status by running `journalctl --follow -u AXIA`, which will show the latest synced block information.
